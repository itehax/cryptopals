...
def histograms(ascii_uppercase, xor_byte, english_probability, get_word_probabilities, trans):
    matches = {}
    for i in range(0xFF + 1):
        try:
            xored = xor_byte(trans,i).decode()
            word_prob = get_word_probabilities(xored.upper())
            matches[i] = sum(abs(e_freq - word_prob[c] * 100) for (c,e_freq) in english_probability.items()) / len(ascii_uppercase)
